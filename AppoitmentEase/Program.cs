...
builder.Services.AddIdentity<EcommerceUser, IdentityRole>() //The parameters here are 1st UserClass (HeroUser)
                                                            //2nd is IdentityRole(here we are using default one as we did not create our own IdentityClass
    .AddEntityFrameworkStores<ProjectEcommerceContext>()
    // .AddEntityFrameworkStores<ProjectEcommerceContext>()    //It shows it is working by using our DbContextClass(here ProjectHeroContext)
